							...
							if (args[1].equalsIgnoreCase("ip")) {
								if (args[2].equalsIgnoreCase("-ip")) {
									...
									if (!banned) {
										...
										Methods.insertInfo(Methods.mysqlConnect(host, port, database, username, password), "ip_bans", "`id`,`ip`,`staff`", id + ",'" + args[3] + "','" + sender.getName() + "'");
									...
									if (plugin.getConfig().contains("playerip." + args[3].toLowerCase())) {
										...
										if (!banned) {
											...
											Methods.insertInfo(Methods.mysqlConnect(host, port, database, username, password), "ip_bans", "`id`,`ip`,`staff`", id + ",'" + ip + "','" + sender.getName() + "'");
											for (Player p : Bukkit.getServer().getOnlinePlayers()) {
												if (p.hasPermission("PalCraftEssentials.command.silent")) {
								...
								if (b) {
									...
									Methods.insertInfo(Methods.mysqlConnect(host, port, database, username, password), "temp_bans", "`id`,`player`,`length`,`staff`,`reason`,`when`,`date`", id + ",'" + args[2] + "','" + args[3] + " " + args[4] + "','" + sender.getName() + "','" + a.toString() + "','" + String.valueOf(System.currentTimeMillis()) + "','" + date + "'");
									...
									if (cn) {
										...
										Methods.editRow(Methods.mysqlConnect(host, port, database, username, password), "player_info", "bans='" + bans + "'", "player='" + args[2] + "'");
										...
										Methods.insertInfo(Methods.mysqlConnect(host, port, database, username, password), "player_info", "`player`,`warnings`,`bans`,`kicks`", "'" + args[2] + "',' ','" + bans + "',' '");
									}
								...
								if (banned) {
									...
									if (cn) {
										...
										Methods.insertInfo(Methods.mysqlConnect(host, port, database, username, password), "player_info", "`player`,`warnings`,`bans`,`kicks`", "'" + p2n + "',' ','" + ban + "',' '");
									}
									for (Player p : Bukkit.getServer().getOnlinePlayers()) {
										if (p.hasPermission("PalCraftEssentials.command.silent")) {
							...
							if (args.length > 2) {
								if (Bukkit.getServer().getOfflinePlayer(args[1]).isOnline()) {
									...
									if (Bukkit.getServer().getPlayer(args[0]).hasPermission("PalCraftEssentials.kick.exempt")) {
										...
										prevKicks += kick;
										if (isInTable) {
											Methods.editRow(Methods.mysqlConnect(host, port, database, username, password), "player_kicks", "player='" + args[1] + "'","kicks='" + prevKicks + kick + "'");
											...
											Methods.insertInfo(Methods.mysqlConnect(host, port, database, username, password), "player_kicks", "`player`,`warnings`,`bans`,`kicks`","'" + args[1] + "',' ',' ','" + kick + "'");
										}
										return true;
									}
								} else {
						...
						if (args.length == 3) {
							if (args[1].equalsIgnoreCase("player")) {
								...
								if (banned) {
									...
									Methods.deleteRow(Methods.mysqlConnect(host, port, database, username, password), "perma_bans", id, "player = '" + args[2] + "'");
									for (Player p : Bukkit.getServer().getOnlinePlayers()) {
										if (p.hasPermission("PalCraftEssentials.command.silent")) {
								...
								if (banned) {
									...
									if (id != -1){
										Methods.deleteRow(Methods.mysqlConnect(host, port, database, username, password), "ip_bans", id, "ip = '" + args[2] + "'");
									}
									for (Player p : Bukkit.getServer().getOnlinePlayers()) {
										if (p.hasPermission("PalCraftEssentials.command.silent")) {
												p.sendMessage(ChatColor.DARK_RED + "Silent - " + sender.getName() +  ChatColor.RED+  ": " + "Unbanning " + args[2]);
								...
								if (id != -1) {
									Methods.deleteRow(Methods.mysqlConnect(host, port, database, username, password), "temp_bans", id, "player = '" + args[2] + "'");
									for (Player p : Bukkit.getServer().getOnlinePlayers()) {
										if (p.hasPermission("PalCraftEssentials.command.silent")) {
